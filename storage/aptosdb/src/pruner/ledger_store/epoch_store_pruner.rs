...
impl DBPruner for EpochStorePruner {
    fn get_pruner_name(&self) -> &'static str {
        EPOCH_STORE_PRUNER_NAME
    }

    fn prune(&self, max_versions: Version) -> anyhow::Result<Version> {
        let current_target_version = self.get_current_target_version(max_versions);
        let mut db_batch = SchemaBatch::new();

        // First prune the ledger info schema before pruning epoch by version as for pruning ledger
        // info schema, we need version to epoch mapping.
        let least_readable_epoch = self.ledger_store.get_epoch(self.least_readable_version())?;
        let target_epoch = self.ledger_store.get_epoch(current_target_version)?;
        self.ledger_store.prune_ledger_info_schema(
            least_readable_epoch,
            target_epoch,
            &mut db_batch,
        )?;
