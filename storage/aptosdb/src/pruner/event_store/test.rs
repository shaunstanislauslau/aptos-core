...
fn generate_random_events() -> impl Strategy<Value = Vec<Vec<ContractEvent>>> {
    let mut rng = rand::thread_rng();
    let num_version = rng.gen_range(0..100);
    let mut result = vec![];
    for _i in 0..num_version {
        let mut inner = vec![];
        let num_events = rng.gen_range(0..100);
        for _j in 0..num_events {
            inner.push(any::<ContractEvent>());
        }
        result.push(inner);
    }
    result
...
proptest! {
    ...
    #[test]
    fn test_put_get_verify(events in generate_random_events()) {
        test_event_store_pruner(events);
    }

}

fn test_event_store_pruner(events: Vec<Vec<ContractEvent>>) {
    ...
    for i in (0..=num_versions).step_by(2_usize) {
        pruner
            .wake_and_wait(
