...
impl LedgerStore {
    ...
    /// Prune the ledger info schema store in DB for epoch range [being, end)
    pub fn prune_ledger_info_schema(
        &self,
        begin_epoch: u64,
        end_epoch: u64,
        db_batch: &mut SchemaBatch,
    ) -> anyhow::Result<()> {
        db_batch.delete_range::<LedgerInfoSchema>(&begin_epoch, &end_epoch)?;
        Ok(())
    }
}
